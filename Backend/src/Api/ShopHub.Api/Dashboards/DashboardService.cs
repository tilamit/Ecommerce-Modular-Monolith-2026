...
/// </summary>
/// <remarks>
/// Lives in the API host rather than in a module because it spans them. It cannot query
/// another module's tables even if it wanted to: every DbContext is <c>internal</c> to its
/// own module, leaving the injected Contracts interfaces as the only reachable surface. A
/// cross-module report is therefore a composition of module APIs rather than a join, and
/// moving Ordering to its own database would not change this file.
/// </remarks>
    ...
    IClock clock)
{
    ...
    internal async Task<AdminDashboardResponse> GetAdminDashboardAsync(CancellationToken cancellationToken) =>
        await cache.GetOrCreateSharedAsync(
