...
internal sealed class Role : AuditableEntity, IAuditableEntity
{
    ...
    /// <summary>
    /// Adds menu grants the role does not have yet and leaves every existing row alone,
    /// whether that row grants or denies.
    /// <para>
    /// This is what the seeder uses, so a grant an administrator changed through access
    /// management survives a restart while a menu item added in a later release still
    /// reaches its default roles. <see cref="ReplaceMenuItems"/> remains the operation
    /// behind the admin screen, where replacing is exactly what is meant.
    /// </para>
    /// </summary>
    public void GrantMenuItemsIfMissing(IEnumerable<Guid> menuItemIds)
    {
        ArgumentNullException.ThrowIfNull(menuItemIds);

        foreach (var menuItemId in menuItemIds.Distinct().Where(id => !_menuItems.Exists(m => m.MenuItemId == id)))
        {
            _menuItems.Add(new RoleMenuItem(Id, menuItemId, isVisible: true));
        }
    }
