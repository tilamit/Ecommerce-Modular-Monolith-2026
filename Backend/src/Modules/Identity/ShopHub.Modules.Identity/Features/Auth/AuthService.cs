    ...
    ILogger<AuthService> logger)
{
    ...
    /// <para>
    /// The presented token is revoked and chained to its replacement. Presenting an already
    /// revoked token revokes the entire family: either it was stolen and replayed, or the
    /// legitimate client replayed one an attacker had used. Both warrant a fresh sign-in.
    /// </para>
    /// <para>
    /// Returns <see cref="RefreshOutcome.Failed"/> rather than throwing. Every SPA boot calls
    /// this endpoint and for an anonymous visitor it is meant to fail, which is not an
    /// exceptional condition.
    /// </para>
    /// </summary>
    internal async Task<RefreshOutcome> RefreshAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var (response, refreshToken, refreshExpiresUtc) = await IssueSessionAsync(
            user,
            existing.FamilyId,
            ipAddress,
            userAgent,
        ...
        return new RefreshOutcome(response, refreshToken, refreshExpiresUtc);
    }
