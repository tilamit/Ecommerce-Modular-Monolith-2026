...
/// tracker still knows what changed and <c>SavedChanges</c> resolves database-generated
/// keys before queueing, since a new row has no id until the save completes. Nothing here
/// touches the database - entries go onto a channel drained by a background service, so
/// auditing stays off the request's critical path and cannot roll back a business
/// transaction.
/// </remarks>
