...
/// <para>
/// Each handler runs in its own DI scope, so a consuming module never shares the publisher's
/// DbContext and cannot end up inside another module's transaction (spec §4.3).
/// </para>
/// <para>
/// A failing handler is logged and swallowed so one bad consumer cannot fail the publisher's
/// request. This is the reliability gap the spec names: if a dropped event matters, the fix
/// is an outbox behind <see cref="IEventBus"/> rather than a change here.
/// </para>
/// </summary>
