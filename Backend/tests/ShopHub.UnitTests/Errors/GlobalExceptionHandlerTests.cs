...
public sealed class GlobalExceptionHandlerTests
{
    ...
    /// <summary>
    /// Minimal APIs raise <see cref="BadHttpRequestException"/> from parameter binding,
    /// before the handler - and therefore before any validator - can run. It describes bad
    /// input, so it must carry the caller's own status rather than falling through to the
    /// 500 that would log a stack trace at Error for `?minPrice=abc`.
    /// </summary>
    [Fact]
    public async Task MalformedRequest_Is400_NotAServerError()
    {
        var (handler, context, captured) = CreateHandler(Environments.Production);

        await handler.TryHandleAsync(
            context,
            new BadHttpRequestException("Failed to bind parameter \"Nullable<decimal> minPrice\" from \"abc\"."),
            CancellationToken.None);

        Assert.Equal(StatusCodes.Status400BadRequest, captured.ProblemDetails!.Status);
        Assert.Equal("malformed_request", captured.ProblemDetails.Extensions["code"]);
        Assert.Contains("minPrice", captured.ProblemDetails.Detail, StringComparison.Ordinal);
    }
