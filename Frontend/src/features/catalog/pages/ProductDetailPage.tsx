...
export const ProductDetailPage = () => {
  ...
  if (productQuery.isError) {
    // Only a 404 means the product is gone. Saying "not found" when the API never answered
    // sends the reader looking for a missing product instead of a stopped backend.
    const isMissing = productQuery.error instanceof ApiError && productQuery.error.status === 404;

    return (
      <ErrorState
        title={isMissing ? 'Product not found' : undefined}
        description={
          isMissing ? 'This product may have been removed or is no longer available.' : undefined
        }
