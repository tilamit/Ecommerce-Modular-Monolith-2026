    ...
    ILogger<IdentitySeeder> logger)
{
        ...
        CancellationToken cancellationToken)
    {
        var adminTitles = new[]
        {
            "Dashboard", "Users", "Categories", "Products", "Offers", "Orders", "Audit Trails", "Access Management",
        };

        var customerTitles = new[] { "My Dashboard", "Purchase History", "My Profile" };

        Grant(roles[AdminRoleName], adminTitles);
        Grant(roles[CustomerRoleName], customerTitles);

        await db.SaveChangesAsync(cancellationToken);

        void Grant(Role role, IEnumerable<string> titles) =>
            role.ReplaceMenuItems(titles.Where(menus.ContainsKey).Select(t => (menus[t].Id, true)));
    }
