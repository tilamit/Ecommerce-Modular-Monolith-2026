...
/// <para>
/// This is the <em>only</em> moment linking is allowed. At checkout the email is unverified,
/// so linking then would be an account-takeover vector (ADR-001); at registration the person
/// has demonstrated control of the address by setting a password on it.
/// </para>
/// <para>
/// Note what it does <b>not</b> do: it never rewrites <c>Orders.UserId</c>. Those orders were
/// placed as guest orders and stay that way - history is not retroactively edited. Only
/// <c>GuestCheckoutProfiles.LinkedUserId</c> is set, which is enough for analytics to join
/// the two.
/// </para>
/// </summary>
